
#include "config.h"




const int MAX_ROAD = 380000;

const int MAX_KEY_NODE = 200000;

const int MAX_LOC_ROAD = 50000; //当前的值是有4643条路

const char * NAV_ROAD_FILE = "./data/map_txt/nav-roadid.txt" ;

const char * LOCID_SEQ_FILE = "data/map_txt/locid_seq.txt";


//const char * txt_root_path = "./data/txt/";
//const char * txt_root_path = "../txt/"; //如果测试可以选择使用这个
const char * txt_root_path = "/media/linuxdata/newcom/data/txt/";

//储存有python生成的环信息
const char * txt_cycle_path = "./data/info/cycles.txt" ;

//locroad  保存的路径文件名

const char * txt_loc_store_path = "./data/info/loc_store.txt";
