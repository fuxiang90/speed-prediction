...
void testNow()
{
    ...
    RoadInfo * head ;
    head = RoadInfoCreat();
    head = RoadInfoInput(head);
    RoadInfoProcess(head);



    string train_file("./data/train/386-6-train-4");
    string test_file("./data/test/386-6-test-4");
    ...
    while(1){
        ...
        scanf("%lf%lf%d%d",&pre_speed,&speed ,&h,&m);
        ...
        speed_arr[0] = 1;
        speed_arr[1] = speed;
        speed_arr[2] = pre_speed;

        speed_arr[3] = pre->history_road[weekday][pos+1];
        speed_arr[4] = p->history_road[weekday][pos+1];

        double next_speed = test.predictNow(speed_arr,weekday,roadid,pos);
